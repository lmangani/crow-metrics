...
function prometheusName(name: MetricName): string {
  ...
  return metricName + "{" + keys.map(k => k + '="' + (metricTags.get(k) || "") + '"').join(",") + "}";
}
...
export function exportPrometheus(snapshot: Snapshot): string {
  ...
  // rename some of the metrics & tags used in distributions.
  const sortedNames = [...snapshot.map.keys()].sort((a, b) => {
    if (a.name != b.name) return a.name.localeCompare(b.name);
    return a.canonical.localeCompare(b.canonical);
  });

  const lines = [ `# generated by crow ${snapshot.registry.version} @ ${snapshot.timestamp}` ];
  ...
  sortedNames.forEach(name => {
    ...
    lines.push(`${prometheusName(name)} ${snapshot.map.get(name)} ${snapshot.timestamp}`);
  })
