...
export interface ExportInfluxDbOptions {
  ...
  // use a field name other than "value"
  fieldName?: string;
 ...
 *     exportInfluxDb(registry.events, { hostname: "influxdb.local:8086", database: "prod" });
 *
 ...
 */
export function exportInfluxDb(events: EventSource<Snapshot>, options: ExportInfluxDbOptions = {}) {
  ...
  events.map(deltaSnapshots()).forEach(async snapshot => {
    // build a plaintext "document" for their made-up format:
    const lines = [ "# generated by crow " + snapshot.registry.version ];
    const map = snapshot.flatten(name => name.format((k, v) => `${k}=${v}`, list => "," + list.join(",")));
