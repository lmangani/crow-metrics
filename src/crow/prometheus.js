...
export class PrometheusObserver {
  ...
  generate() {
    ...
    const lines = [ `# generated by crow ${this.lastSnapshot.registry.version}` ];
...
export function prometheusExporter(express, registry) {
  const prometheusObserver = new PrometheusObserver();
  registry.addObserver(prometheusObserver.observer);

  const router = express.Router();
  router.get("/", (request, response) => {
    response.set("Content-Type", "text/plain; version=0.0.4");
    response.send(prometheusObserver.generate());
  });
